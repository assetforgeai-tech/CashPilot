...
def classify_verdict(verdict: str, reason: str = "") -> str:
    ...
    if value == "CID_SET":
        return "eligible"
    if value == "BLACKLIST" or str(reason or "").strip() == "earnapp_blacklist":
        return "blocked"
    if value == "DECLINE":
    ...
    try:
        ...
        while True:
            ...
            if msg_type == "ipc_post" and command == "tunnel_init_decline":
                ...
                result.update(verdict="BLACKLIST" if reason == "earnapp_blacklist" else "DECLINE", reason=reason)
                break
            ...
            if msg_type == "ipc_call" and command == "tunnel_init":
                ...
                reply = {"type": "ipc_result", "cmd": command, "cookie": message.get("cookie"), "msg": identity}
                writer.write(encode_client_text_frame(json.dumps(reply, separators=(",", ":"))))
                await writer.drain()
            elif msg_type == "ipc_call" and command in {"tunnel_init_done", "check_status_send"}:
                body = {"idle": False} if command == "check_status_send" else {"ok": True}
                reply = {"type": "ipc_result", "cmd": command, "cookie": message.get("cookie"), "msg": body}
                writer.write(encode_client_text_frame(json.dumps(reply, separators=(",", ":"))))
                await writer.drain()
    except TimeoutError:
        result.update(verdict="TIMEOUT", reason=f"no decline/cid in {int(wait_seconds * 1000)}ms")
