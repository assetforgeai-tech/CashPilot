...
class ProviderRuntime:
    ...
    # Keep migration exceptions visible in the provider matrix instead of
    # silently treating the compatibility backend as fully standardized.
    proxy_transport_exception: str = "shared sing-box sidecar; in-container migration pending provider canary"
...
PROVIDERS: dict[str, ProviderRuntime] = {
    ...
    "iproyal": ProviderRuntime(
        ...
        proxy_transport_exception="shared sing-box sidecar; in-container migration pending provider canary",
        policy_group="pawns",
    ...
    "spide": ProviderRuntime("spide", "spide.py", "", ("direct", "proxy"), "dashboard_only", topology="slot_both"),
...
def proxy_contract(slug: str) -> str:
    """Return the shared proxy-network contract without changing provider policy."""
    provider = get(str(slug or "").strip().lower())
    if provider is None or "proxy" not in provider.modes:
        return "direct-only"
    # Both the EarnApp in-container route and the migration sidecar implement
    # this same externally observable contract; transport backend stays separate.
...
def catalog_runtime(slug: str) -> dict[str, object]:
    ...
    return {
        ...
        "proxy_transport_exception": provider.proxy_transport_exception,
        "proxy_contract": proxy_contract(slug),
        ...
        "cardinality_policy": {
            "desired": ("bootstrap_public_ipv4" if provider.topology.startswith("slot_") else "dedicated_runtime"),
