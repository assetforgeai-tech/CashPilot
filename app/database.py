...
async def _ensure_earnapp_account_operation_schema(db: Any, applied: list[str] | None = None) -> None:
    ...
    if await _table_columns(db, "earnapp_account_operations") != _EARNAPP_CHILD_COLUMNS["earnapp_account_operations"]:
        return
    await db.execute(
        "CREATE INDEX IF NOT EXISTS idx_earnapp_account_operations_claim "
        "ON earnapp_account_operations(account_id, state, not_before, id)"
...
async def enqueue_earnapp_account_operation(
    ...
    *,
    ...
    try:
        ...
        await db.commit()
        row = await (
            await db.execute("SELECT * FROM earnapp_account_operations WHERE operation_key = ?", (operation_key,))
        ).fetchone()
        return dict(row)
    finally:
        await db.close()
...
async def claim_earnapp_account_operation(
    ...
    *,
    ...
    try:
        ...
        await db.commit()
        claimed = await (
            await db.execute("SELECT * FROM earnapp_account_operations WHERE id = ?", (row["id"],))
        ).fetchone()
...
async def create_earnapp_staged_identity_profile(
    stage_slug: str, logical_node_id: str, platform: str
) -> dict[str, str]:
        ...
        try:
            row = await (
                await db.execute(
                    ...
                    "FROM earnapp_staged_identity_profiles WHERE stage_slug = ?",
                    (stage_slug,),
                )
            ).fetchone()
...
async def create_earnapp_replacement_transaction(
    ...
    *,
    ...
    try:
        ...
        await db.commit()
        row = await (
            await db.execute("SELECT * FROM earnapp_replacement_transactions WHERE logical_node_id = ?", (node_id,))
        ).fetchone()
    finally:
        await db.close()
    if not row:
...
async def delete_earnapp_replacement_transaction(logical_node_id: str) -> bool:
    ...
    db = await _get_db()
    try:
        cursor = await db.execute("DELETE FROM earnapp_replacement_transactions WHERE logical_node_id = ?", (node_id,))
        await db.commit()
        return bool(cursor.rowcount)
    finally:
        await db.close()
    ...
    async with _earnapp_lock():
        ...
        try:
            ...
            if platform not in {"macos", "ios", "ubuntu"}:
                ...
                return None
            country_clause = (
                "AND length(trim(coalesce(pe.country_code, ''))) = 2 AND upper(trim(pe.country_code)) GLOB '[A-Z][A-Z]'"
            )
            candidate = await (
                await db.execute(
                    f"""
    ...
    *,
    ...
    async with _earnapp_lock():
        ...
        try:
            ...
            instance = await (
                await db.execute(
                    "SELECT status, worker_id FROM provider_instances WHERE slug='earnapp' AND instance_id = ?",
                    (node_id,),
                )
            ).fetchone()
            if (
    ...
    *,
    ...
    async with _earnapp_lock():
        ...
        try:
            ...
            rows = await (
                await db.execute(
                    "SELECT instance_id, status, updated_at FROM provider_instances WHERE worker_id=? AND slug != 'earnapp'",
                    (int(worker_id),),
                )
            ).fetchall()
            marked: list[str] = []
            ...
            for row in rows:
                ...
                if not instance_id or instance_id in reported:
                    ...
                    continue
                if str(row["status"] or "") == "missing_once":
                    # Two adjacent heartbeats are too weak to release a proxy:
                    # worker upgrades and Docker inventory races can hide a
                    # still-running runtime briefly. Keep authority durable for
                    # one hour so normal redeploy can reattach the row/lease.
                    hold = await (
                        await db.execute(
                            "SELECT datetime(?, '+1 hour') > datetime('now') AS active",
                            (str(row["updated_at"] or ""),),
                        )
                    ).fetchone()
                    if hold and bool(hold["active"]):
                        continue
                    await db.execute(
