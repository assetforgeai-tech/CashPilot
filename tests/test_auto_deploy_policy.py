...
def test_heartbeat_auto_deploy_retries_failed_provider_on_same_worker():
    async def run():
        main._WORKER_HEARTBEAT_STREAKS.clear()
        main._NKN_AUTO_DEPLOY_DONE.add(7)
        main._EARNAPP_AUTO_DEPLOY_DONE.add(7)
        services = [
            {
                "slug": "iproyal",
                "status": "active",
                "docker": {"image": "iproyal/pawns-cli:latest"},
                "deploy": {},
            }
        ]
        with (
            ...
            patch.object(
                ...
                AsyncMock(return_value=[{"slug": "iproyal", "status": "failed"}]),
            ),
            patch.object(main.catalog, "get_services", return_value=services),
            patch.object(main, "_spawn") as spawn,
        ):
