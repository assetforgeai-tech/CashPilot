...
def test_nkn_lxd_deploy_uses_long_worker_timeout_without_snapshot():
    async def run():
        with (
            patch.object(database, "get_worker", AsyncMock(return_value={"id": 7, "client_id": "worker-a"})),
            patch.object(main, "_worker_public_ip_slots", AsyncMock(return_value=[_slot("ipv4-001", "8.8.8.8")])),
            patch.object(
                database,
                "lease_nkn_wallet",
                AsyncMock(return_value=_lease(1, "worker-a:nkn:ipv4-001", "8.8.8.8")),
            ),
            patch.object(database, "get_provider_instance", AsyncMock(return_value=None)),
            patch.object(main, "_nkn_chaindb_snapshot_for_deploy", AsyncMock(return_value=None)),
            patch.object(
...
def test_nkn_worker_timeout_keeps_wallet_lease_for_same_slot_retry():
    async def run():
        with (
            patch.object(database, "get_worker", AsyncMock(return_value={"id": 7, "client_id": "worker-a"})),
            patch.object(main, "_worker_public_ip_slots", AsyncMock(return_value=[_slot("ipv4-001", "8.8.8.8")])),
            ...
            patch.object(database, "get_provider_instance", AsyncMock(return_value=None)),
            patch.object(main, "_nkn_chaindb_snapshot_for_deploy", AsyncMock(return_value=None)),
            patch.object(main, "_proxy_worker_nkn_deploy", AsyncMock(side_effect=main.HTTPException(503, "timeout"))),
            patch.object(database, "save_provider_instance", AsyncMock()),
...
def test_nkn_worker_400_keeps_wallet_lease_when_lxd_may_have_started():
    async def run():
        with (
            patch.object(database, "get_worker", AsyncMock(return_value={"id": 7, "client_id": "worker-a"})),
            patch.object(main, "_worker_public_ip_slots", AsyncMock(return_value=[_slot("ipv4-001", "8.8.8.8")])),
            ...
            patch.object(main, "_nkn_chaindb_snapshot_for_deploy", AsyncMock(return_value=None)),
            patch.object(
                main, "_proxy_worker_nkn_deploy", AsyncMock(side_effect=main.HTTPException(400, "route failed"))
            ),
            patch.object(database, "save_provider_instance", AsyncMock()),
            patch.object(database, "release_nkn_wallet", AsyncMock()) as release,
        ):
