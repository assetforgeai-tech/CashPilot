...
def test_init_does_not_guess_owner_for_conflicting_legacy_egress(tmp_path):
    async def run():
        with patch.object(database, "DB_DIR", tmp_path), patch.object(database, "DB_PATH", tmp_path / "conflict.db"):
            await database.init_db()
