...
def test_migrate_legacy_earnapp_ubuntu_uses_verified_image_and_rolls_forward():
    client = MagicMock()
    old = MagicMock(id="old-id", name="cashpilot-earnapp-ubuntu", status="running")
    old.labels = {
        orchestrator.LABEL_MANAGED: "true",
        orchestrator.LABEL_SERVICE: "earnapp-ubuntu",
        "cashpilot.provider": "earnapp",
        "cashpilot.earnapp.logical_node_id": "earnapp-ubuntu",
    ...
    old.attrs = {
        "Config": {
            "Image": "cashpilot/earnapp-ubuntu:legacy",
            "Env": ["PROXY_TYPE=SOCKS5", "PROXY_CREDENTIALS=secret", "NODE_TLS_REJECT_UNAUTHORIZED=0"],
            "Labels": old.labels,
            "Cmd": ["/entrypoint"],
        },
        "HostConfig": {
        ...
        "Mounts": [{"Type": "volume", "Name": "earnapp-ubuntu-data", "Destination": "/etc/earnapp", "RW": True}],
    }
    replacement = MagicMock(id="new-id", status="running")
    client.containers.create.return_value = replacement
    image = MagicMock()
    image.attrs = {"Config": {"Labels": earnapp_runtime.required_image_labels("ubuntu")}}
    ...
    with (
        ...
        patch.object(
            orchestrator, "wait_for_service_egress", return_value={"probe_ok": True, "observed_egress_ip": "198.51.100.8"}
        ),
        patch.object(
    ...
    assert kwargs["image"] == earnapp_runtime.UBUNTU_RUNTIME_IMAGE
    assert kwargs["volumes"] == {"earnapp-ubuntu-data": {"bind": "/etc/earnapp", "mode": "rw"}}
    assert kwargs["labels"]["custom.label"] == "keep"
    assert kwargs["environment"]["NODE_TLS_REJECT_UNAUTHORIZED"] == "1"
    assert kwargs["environment"]["PROXY_CREDENTIALS"] == "secret"
    old.remove.assert_called_once_with(force=True)


def test_migrate_legacy_earnapp_ubuntu_rolls_back_when_network_evidence_fails():
    client = MagicMock()
    old = MagicMock(id="old-id", name="cashpilot-earnapp-ubuntu", status="running")
    old.labels = {
        orchestrator.LABEL_MANAGED: "true",
        ...
        "cashpilot.earnapp.device_id": "sdk-node-" + "b" * 32,
    }
    old.attrs = {
        "Config": {"Image": "legacy", "Env": [], "Labels": old.labels},
        "HostConfig": {"NetworkMode": "bridge", "RestartPolicy": {"Name": "always"}},
        "Mounts": [{"Type": "volume", "Name": "state", "Destination": "/etc/earnapp", "RW": True}],
    }
    replacement = MagicMock(id="new-id", status="running")
    client.containers.create.return_value = replacement
    image = MagicMock()
