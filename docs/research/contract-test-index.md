...
| NKN Wallet | Owner-only wallet import and inventory/list boundary; no active provider, lease or worker runtime flow | `tests/test_nkn_wallets_module.py` | Source/test evidence; live inventory unknown; runtime absent |
...
- Không có live NKN inventory snapshot; current source cũng chưa có NKN provider/lease/runtime contract để kiểm chứng completion.
