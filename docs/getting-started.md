...
Of these, **15 providers** retain Docker/runtime catalog metadata, and **9 collectors** use the shared collector registry. EarnApp uses its separate account-scoped collector, but its hosted canary/runtime lane is currently `RUNTIME_DISABLED`; providers without collection support still need dashboard/manual verification for earnings.
